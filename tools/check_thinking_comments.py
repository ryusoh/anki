...
Exit code 1 (and one line per violation) if anything is found. Wired into the
VERIFY_GATE as the ``thinking-check`` make target.
"""
...
SCAN_EXTENSIONS = (".py", ".js", ".cjs", ".mjs", ".css")

# Third-party trees are not ours to police (AGENTS.md non-negotiable #5).
EXCLUDED_PARTS = ("libaddon/", "_vendor/", "_vendor_legacy/", "node_modules/", "assets/vendor/")
...
def scan_js_comments(src):
    """Yield (lineno, comment_text) for JS comments matching the pattern.
...
def find_violations(paths):
    ...
    for path in paths:
        ...
        if path.endswith(".py"):
            ...
            for lineno, text in scan_js_comments(src):
                yield f"{path}:{lineno}: thinking-out-loud comment: // {text}"
...
def main():
    ...
    if violations:
        print(
            ...
            "(AGENTS.md non-negotiable #10). Delete the reasoning — "
