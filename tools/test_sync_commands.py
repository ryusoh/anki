...
def test_parse_markdown_no_frontmatter():
    yaml_data, body = sync_commands.parse_markdown("just body")
    assert yaml_data == {}
    assert body == "just body"


def test_parse_markdown_with_frontmatter():
    yaml_data, body = sync_commands.parse_markdown("---\ndescription: test\n---\nbody text")
...
def test_generate_replaces_args(tmp_path):
    ...
    sync_commands.generate(str(skills_dir), str(target_dir))
...
def test_check_returns_true_on_match(tmp_path):
    ...
    assert sync_commands.check(str(skills_dir), str(target_dir))


def test_format_generated_commands_empty_dir(tmp_path):
    target_dir = tmp_path / "commands"
    target_dir.mkdir()
    sync_commands.format_generated_commands(str(target_dir))
...
def test_ensure_skills_symlink_exists(tmp_path):
    ...
    sync_commands.ensure_skills_symlink(str(target_link))
    assert target_link.read_text() == "dummy"


def test_main_generate(monkeypatch, capsys):
    monkeypatch.setattr("sys.argv", ["sync_commands.py"])
    monkeypatch.setattr(sync_commands, "generate", lambda s, t: print("mock_generate"))
    monkeypatch.setattr(sync_commands, "ensure_skills_symlink", lambda: print("mock_symlink"))
...
def test_main_check_pass(monkeypatch, capsys):
    import pytest

    monkeypatch.setattr("sys.argv", ["sync_commands.py", "--check"])
    monkeypatch.setattr(sync_commands, "check", lambda s, t: True)
...
def test_main_check_fail(monkeypatch, capsys):
    import pytest

    monkeypatch.setattr("sys.argv", ["sync_commands.py", "--check"])
    monkeypatch.setattr(sync_commands, "check", lambda s, t: False)
