...
CLI is deprecated in favour of Antigravity) — this script does not read or
write it, so those files are frozen and edited by hand if ever needed.
"""
...
def format_generated_skills() -> None:
    """Format generated skills with prettier so output matches `make fmt`.
    ...
    try:
        subprocess.run(
            ["npx", "prettier", "--write", "--ignore-path", ".gitignore", SKILLS_DIR],
            cwd=WORKSPACE_ROOT,
            check=True,
            capture_output=True,
            text=True,
