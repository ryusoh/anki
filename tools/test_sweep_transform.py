...
def test_main(monkeypatch, tmp_path, capsys):
    ...
    def mock_snapshot(c): return str(col)
    def mock_remove(c): pass
    monkeypatch.setattr("sweep_transform.snapshot_collection", mock_snapshot)
    monkeypatch.setattr("sweep_transform.remove_snapshot", mock_remove)

    monkeypatch.setattr("sys.argv", ["sweep_transform.py", "sys:exit", "--collection", str(col)])
    def mock_transform(s): return s.replace("foo", "bar")
    monkeypatch.setattr("sweep_transform.load_transform", lambda x: mock_transform)
...
def test_main_with_limit(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr("sys.argv", ["sweep_transform.py", "sys:exit", "--collection", str(col), "--limit", "1"])
    def mock_transform(s): return s.replace("foo", "bar")
    monkeypatch.setattr("sweep_transform.load_transform", lambda x: mock_transform)
