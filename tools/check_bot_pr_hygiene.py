...
wording in AGENTS.md and their persona, but wording did not stop PR #494:
...
def _is_test_path(path: str) -> bool:
    parts = path.split('/')
    name = parts[-1]
    return 'tests' in parts or name.startswith('test_') or '.test.' in name
    ...
    for sha in reversed(revs):
        ...
        for added, deleted, path in rows:
            ...
            if _is_stray_artifact(path):
                violations.append(f'{sha[:8]} stray artifact: {path} must not be committed')
            if path == 'eslint-suppressions.json' or path.endswith('/eslint-suppressions.json'):
                if not subject.startswith('refactor'):
                    violations.append(
                        f'{sha[:8]} lane violation: only Refactoring (refactor) may touch {path}'
                    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description='Fail on empty commits, placeholder files, test deletions, stray artifacts, or complexity ratchet violations in bot-authored commits.',
    )
    parser.add_argument(
        '--base',
        default='origin/main',
    ...
    try:
        ...
        try:
            ...
            print('❌ neither origin/main nor main found', file=sys.stderr)
            return 2
    ...
    if violations:
        print('❌ Bot PR hygiene violations (AGENTS.md non-negotiable #11):')
