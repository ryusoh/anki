...
# Keep in sync with the persona set in .jules/ (pinned by tools/test_prior_prs.py).
KNOWN_LANES = ('bolt', 'janitor', 'palette', 'refactoring', 'sentinel', 'testpilot', 'typist')
...
def format_stats(stats: dict[str, Any]) -> str:
    ...
    for lane in sorted(
        ...
        lines.append(
            f"{lane:<14}{bucket['open']:>5}{bucket['merged']:>7}{bucket['closed']:>7}"
            f'  {_accept_rate(bucket)}'
        )
    total = stats['total']
    lines.append(
        f"{'TOTAL':<14}{total['open']:>5}{total['merged']:>7}{total['closed']:>7}"
        f'  {_accept_rate(total)}'
    )
    if total['reasons']:
        lines.append(
