...
**Learning:** When testing ESM modules that rely on DOM globals (like `window` or `document`) in a pure Node environment, these globals must be mocked globally *before* importing the target module dynamically via `await import(...)`. Otherwise, top-level evaluation of the module will throw a ReferenceError.
