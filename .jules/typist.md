...
`npx tsc -p jsconfig.json` (`make typecheck-js`) is blocking in `VERIFY_GATE`
(`make verify` / `make precommit SKIP=1`). Each run, do exactly one of the
following, checked in order. **No runtime behavior change, ever.**
...
0. **Early exit** — if your run's driving goal is already satisfied by the
   ...
   **end the run with no PR** (AGENTS.md non-negotiable #11). A no-op summary
...
2. **Expand** — if the whitelist is clean, grow it: run the expansion scan (see
   ...
   job.** Check `docs/js-typing-strategy.md`'s Status section first — it may
   already record a known error in a candidate TARGET (e.g. a duplicate-key bug
   discovered ahead of time) that you should fix as part of that file's PR.
...
"js/vendor"]`, confirm `make typecheck-js` is green, and update the status in
...
- You must NOT touch: runtime logic, tests (Testpilot's lane), CSS, Python,
  `js/vendor/**` (vendored, permanently excluded), `review_heatmap/libaddon/`
  (vendored), or any source file other than the selected TARGET. One file per
  run.
...
- Expansion scan: copy `jsconfig.json` to a temp file (delete it before
  ...
  `exclude` set to `["node_modules", "js/vendor"]`, run `npx tsc -p <temp>`,
  and tally errors per file. Record the total — the PR body reports it before
  → after.
- Resolve every error in TARGET with correct JSDoc / `@typedef`. Put shared
  types in `js/types/*.d.ts` (type-only, never shipped).
...
- **`#js/*` and `#ui/*` subpath imports (from `package.json`'s `"imports"`
  field) already resolve correctly** under `moduleResolution: "bundler"` — no
  extra `paths` entries needed in `jsconfig.json`. Adding a file that imports
  via `#js/...` pulls its import graph into the check transitively (this is
  how the `js/config/assetClasses.js` duplicate-key bug in
  `docs/js-typing-strategy.md` was found before it was even whitelisted).
- If a whitelisted file assigns to `window.<Something>` (global-script style,
  no `import`/`export`), TS needs an `interface Window { ... }` augmentation
  in `js/types/globals.d.ts` — never widen to `any`.
- Vendored libraries (`js/vendor/**`) are out of the program; if a vendored
...
- `make typecheck-js` green, and `make precommit SKIP=1` green.
