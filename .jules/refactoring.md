...
- **Dead Code Pruning:** Refactored multiple high complexity functions (`radon cc`) across the project, including `main` in `data/anki/migrate-hash-map.py` and `calculate_future_due` in `data/anki/generate_custom_stats.py`, to improve code readability and maintainability.
- **Silent Failure Audit:** Replaced generic empty `except` blocks with exception logging and bound exception references in `awesome_tts`, `auto_wiktionary`, `data/anki/upload-to-r2`, and other core python utilities to maintain context for failing scenarios.
