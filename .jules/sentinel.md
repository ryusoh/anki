...
## 2026-05-18 - Prevent DOM-based XSS by removing innerHTML in review heatmap views

**Vulnerability:** Emptying DOM elements using `container.innerHTML = ""` and parsing markup using `<template>.innerHTML = heatmapHtml` in `review_heatmap/views.py` to inject the heatmap into the webview.
**Learning:** While `heatmapHtml` is generated by Python and might be safe, assigning dynamically built strings to `innerHTML` violates modern SAST rules and risks accidental XSS introduction. Safe DOM APIs should be the standard everywhere.
**Prevention:** To prevent DOM-based XSS vulnerabilities and comply with strict SAST linters, avoid assigning strings to `.innerHTML`. Instead, use safer native DOM APIs like `.textContent`, `.replaceChildren()`, or `DOMParser().parseFromString(..., 'text/html')`.
