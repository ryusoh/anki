...
from graph.parser import extract_fields, tokenize, group_by_deck, get_front_field
from graph.builder import build_graph, build_per_deck_graphs, _compute_pagerank, export_to_dict, get_top_nodes, get_isolated_nodes, get_hub_nodes
...
def load_notes_with_decks():
    ...
    r2_staged = Path(__file__).parent.parent / "data" / "cloudflare" / "collection" / "notes.json.gz"
    if r2_staged.exists():
...
def print_top_notes(graph, deck_name, top_n=10):
    ...
    for rank, (node_id, data) in enumerate(top, 1):
        front = data.get('front', 'Unknown')[:28] + '..' if len(data.get('front', '')) > 30 else data.get('front', 'Unknown')
        pagerank = data.get('pagerank', 0)
        tags = data.get('tags', '')[:18] + '..' if len(data.get('tags', '')) > 20 else data.get('tags', '')
...
def print_hub_notes(graph, deck_name, threshold=0.01):
    ...
    for rank, (node_id, data) in enumerate(hubs, 1):
        front = data.get('front', 'Unknown')[:28] + '..' if len(data.get('front', '')) > 30 else data.get('front', 'Unknown')
        pagerank = data.get('pagerank', 0)
...
def compare_decks(graphs):
    ...
    for deck_name, graph in sorted(graphs.items()):
        ...
        print(f"{deck_name[:29]:<30} {num_nodes:<8} {num_edges:<8} {density:<10.4f} {top_front:<40}")
...
def main():
    ...
    parser.add_argument('-d', '--deck', type=str, help='Analyze specific deck (aliases: J, C, E, S, T, F)')
    ...
    parser.add_argument('-f', '--format', type=str, default='json', choices=['json', 'graphml'], help='Export format')
