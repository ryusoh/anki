...
import sys, time, subprocess, argparse, webbrowser
from pathlib import Path
from datetime import datetime

CONFIG_FILE = Path('/Users/lz/Library/Application Support/Anki2/addons21/graph/.incremental_config.json')
EXPORT_SCRIPT = Path('/Users/lz/Library/Application Support/Anki2/addons21/graph/incremental_export.py')
...
def increment():
    ...
    result = subprocess.run(
        [sys.executable, str(EXPORT_SCRIPT), '--next'],
        capture_output=True,
        text=True
    )
...
def main():
    ...
    parser.add_argument('--max', '-m', type=int, default=None,
                       help='Maximum sample size (default: unlimited)')
    parser.add_argument('--auto-refresh', '-r', action='store_true',
                       help='Auto-refresh browser')
    parser.add_argument('--once', '-o', action='store_true',
                       help='Run once and exit')
    
    args = parser.parse_args()
    
    print(f"👁️  Watcher started")
    ...
    try:
        while True:
            ...
            if current_size != last_size:
                print(f"[{datetime.now().strftime('%H:%M:%S')}] Size changed: {last_size:,} → {current_size:,}")
