...
SECURITY: Credentials loaded from ~/.anki-r2/credentials (NEVER hardcode!)
"""
...
def load_r2_credentials():
    ...
    for key in required:
        if key not in creds:
            print(f"❌ Missing {key} in ~/.anki-r2/credentials")
            sys.exit(1)
...
ACCOUNT_ID = CREDS['R2_ACCOUNT_ID']
ACCESS_KEY = CREDS['R2_ACCESS_KEY_ID']
SECRET_KEY = CREDS['R2_SECRET_ACCESS_KEY']
