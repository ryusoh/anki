...
function resolveOptions(pluginOptions) {
  const globalConfig =
    ...
    {};
  return deepMerge(
    deepMerge(DEFAULT_OPTIONS, globalConfig),
    pluginOptions || {},
  );
}
...
function ensureState(chart, options) {
  ...
  if (!particlesEnabled) {
    ...
    !chart.$glass3d.energyParticles ||
    chart.$glass3d.energyParticles.length === 0
  ) {
...
function updatePointerState(state, chart, options) {
  ...
  state.pointerSmoothed.x +=
    (state.pointerTarget.x - state.pointerSmoothed.x) * damping;
  state.pointerSmoothed.y +=
    (state.pointerTarget.y - state.pointerSmoothed.y) * damping;
}
...
function drawSideWall(ctx, arc, depth, options, lightVec, pointer) {
  ...
  const lightInfluence = Math.max(
    0,
    lightVec.x * Math.cos(sliceCenterAngle) +
      lightVec.y * Math.sin(sliceCenterAngle),
  );
  ...
  gradient.addColorStop(
    0,
    `rgba(255, 255, 255, ${Math.min(0.95, topOpacity + highlightBoost)})`,
  );
  gradient.addColorStop(
    1,
    `rgba(0, 0, 0, ${Math.max(0, bottomOpacity - highlightBoost * 0.6)})`,
  );
  ...
  if (innerRadius > 0) {
    ...
    innerGradient.addColorStop(
      0,
      `rgba(255, 255, 255, ${0.25 + highlightBoost * 0.5})`,
    );
...
function updateEnergyParticles(state, delta, options) {
  ...
  const baseSpeed =
    (options.reflection?.speed ?? 0.05) *
    (options.electric?.particleSpeedMultiplier ?? 1);
  state.energyParticles.forEach((particle, idx) => {
    const variance = 0.8 + (idx % 5) * 0.12;
    particle.angle =
      (particle.angle + delta * baseSpeed * 6 * particle.speed * variance) %
      (Math.PI * 2);
    particle.radiusFactor =
      0.75 +
      Math.sin(state.phase * Math.PI * 2 + particle.flickerOffset) * 0.05;
  });
}
...
function drawElectricTrail(
  ...
  for (let i = 0; i < arcCount; i += 1) {
    ...
    const localPhase =
      state.continuousPhase * speedMultiplier + (i / arcCount) * 0.65;
...
function drawEnergyParticles(
  ...
  const colors =
    ...
    Array.isArray(options.electric.particleColors)
      ? options.electric.particleColors
  ...
  state.energyParticles.forEach((particle, idx) => {
    ...
    const flicker =
      0.5 + 0.5 * Math.sin(state.phase * Math.PI * 4 + particle.flickerOffset);
...
function drawAmbientGlow(
  ...
  const innerColor = applyAlpha(
    glow.innerColor,
    innerOpacity * (0.6 + pulse * 0.4),
  );
...
export const glass3dPlugin = {
  ...
  beforeDatasetsDraw(chart, args, pluginOptions) {
    ...
    const depth =
      depthValue !== undefined ? depthValue : Math.max(outerRadius * 0.12, 12);
