...
function mergeChartSettings(base, override = {}) {
  return {
    ...
    mobileHaloOscillation:
      override.mobileHaloOscillation ?? base.mobileHaloOscillation,
    desktopHaloOscillation:
      override.desktopHaloOscillation ?? base.desktopHaloOscillation,
...
function drawGlow(ctx, series, options, settings) {
  ...
  const oscillation =
    0.5 + 0.5 * Math.sin(appliedPhase * settings.oscillationSpeed);
  const minPulseRadius = Math.max(settings.minPulseRadius, lineWidth / 2);
  const haloBase = isMobile
    ? settings.mobileHaloBase
    : settings.desktopHaloBase;
  ...
  if (tailCount > 1) {
    ...
    gradient.addColorStop(
      1,
      `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${settings.tailOpacity})`,
    );
    ...
    ctx.shadowBlur = isMobile
      ? settings.mobileShadowBlur
      : settings.desktopShadowBlur;
    ctx.beginPath();
  ...
  ctx.shadowColor = `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${settings.shadowOpacity})`;
  const glowBlur =
    (isMobile ? settings.mobileShadowBlur : settings.desktopShadowBlur) * 0.8;
