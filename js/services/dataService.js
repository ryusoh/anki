...
function lightenHexToRgba(hex, lightenFactor, alpha) {
  ...
  const light = Math.max(
    0,
    Math.min(1, Number.isFinite(lightenFactor) ? lightenFactor : 0.5),
  );
  /* istanbul ignore next: defensive parameter validation */
  const targetAlpha = Number.isFinite(alpha)
...
async function loadAnalysisTickerPaths() {
  ...
  const entries = Array.isArray(indexPayload?.tickers)
    ? indexPayload.tickers
    : [];
    ...
    await Promise.all(
      uniqueSymbols.map(async (symbol) => {
        ...
        try {
          ...
          logger.warn(
            `Failed to load analysis market data for ${symbol}:`,
            error,
          );
        }
      }),
...
function processAndEnrichHoldings(holdingsDetails, prices) {
  ...
  let totalPnl = 0;
  const enrichedHoldings = Object.entries(holdingsDetails).map(
    ([ticker, details]) => {
...
function createHoldingRow(
  ...
  const allocationPercentage =
    totalPortfolioValueUSD > 0
      ? (holding.currentValue / totalPortfolioValueUSD) * 100
      : 0;
...
function updateTableAndPrepareChartData(
  ...
  sortedHoldings.forEach((holding, index) => {
    ...
    const resolvedSrc =
      typeof originalLogoInfo.src === "string" &&
      originalLogoInfo.src.startsWith("http")
...
function calculateRealtimePnl(
  ...
  const pnlUSD = calculatePnlPercentage(
    dailyChangeUSD,
    baselineTotals.totalUSD,
  );
  const pnlCNY = calculatePnlPercentage(
    dailyChangeCNY,
    baselineTotals.totalCNY,
  );
  const pnlJPY = calculatePnlPercentage(
    dailyChangeJPY,
    baselineTotals.totalJPY,
  );
  const pnlKRW = calculatePnlPercentage(
    dailyChangeKRW,
    baselineTotals.totalKRW,
  );
...
function computeMonthlyPnl(processedData) {
  ...
  for (let idx = 0; idx < sortedMonthKeys.length; idx++) {
    ...
    const absoluteChangeCNY =
      (lastEntry.totalCNY || 0) - (baseEntry.totalCNY || 0);
    const absoluteChangeJPY =
      (lastEntry.totalJPY || 0) - (baseEntry.totalJPY || 0);
    const absoluteChangeKRW =
      (lastEntry.totalKRW || 0) - (baseEntry.totalKRW || 0);
    ...
    const percentChangeUSD = calculatePercentChange(
      absoluteChangeUSD,
      baseTotal,
    );
    const percentChangeCNY = calculatePercentChange(
      absoluteChangeCNY,
      baseEntry.totalCNY || 0,
    );
    const percentChangeJPY = calculatePercentChange(
      absoluteChangeJPY,
      baseEntry.totalJPY || 0,
    );
    const percentChangeKRW = calculatePercentChange(
      absoluteChangeKRW,
      baseEntry.totalKRW || 0,
    );
...
export async function loadAndDisplayPortfolioData(
  ...
  try {
    ...
    const marketRatiosByTicker =
      await fetchMarketRatiosForTickers(tickerSymbols);
    ...
    document.getElementById("total-portfolio-value-in-table").textContent =
      formatCurrency(
        totalPortfolioValueUSD,
        currentCurrency,
        exchangeRates,
    ...
    const totalPnlPercentage =
      totalPortfolioCostUSD !== 0
        ? (totalPnlUSD / totalPortfolioCostUSD) * 100
        : 0;
    ...
    const pnlColor =
      totalPnlUSD >= 0 ? COLORS.POSITIVE_PNL : COLORS.NEGATIVE_PNL;
