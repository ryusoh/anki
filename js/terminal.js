  ...
  input.addEventListener("keydown", (event) => {
    ...
    if (event.key === "Tab") {
      ...
      if (!currentInput) {
        ...
        appendLine(
          "Available commands: " +
            allCommands.slice(0, 20).join(", ") +
            (allCommands.length > 20 ? "..." : ""),
          "muted",
        );
        return;
      }
      ...
      if (
        ...
        autocompleteState.currentIndex =
      ...
      // Apply suggestion
      input.value =
        autocompleteState.suggestions[autocompleteState.currentIndex];

      // Show remaining suggestions if multiple
      if (suggestions.length > 1) {
        appendLine(
          `Tab ${autocompleteState.currentIndex + 1}/${suggestions.length}: ${suggestions.join(" | ")}`,
          "muted",
        );
      }
    }
    ...
    ([customReady, reviewReady]) => {
      if (!customReady) {
        ...
        if (empty) {
          ...
          empty.textContent =
            "データがありません。Anki を起動してからこのページを再読み込みしてください。";
        }
      }
    },
  );
