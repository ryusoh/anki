...
function getPreferences(config) {
  ...
  return model.preferences && typeof model.preferences === "object"
    ? model.preferences
    : {};
}
...
export function extractScenarioTitles(markdown) {
  ...
  while (match) {
    ...
    const scenarioMatch = heading.match(
      /\b(Bull|Base|Bear)\b(?:\s+Case)?\s*(?:[-–]\s*(.+))?/i,
    );
    if (scenarioMatch) {
...
function normalizeScenario(scenario) {
  ...
  const precomputedMultipleRaw = Number(
  ...
  const precomputedMultiple = Number.isFinite(precomputedMultipleRaw)
    ...
    : null;
  const precomputedCagrRaw = Number(
    scenario.precomputedCagr ?? scenario.scenarioCagr,
  );
  const precomputedCagr = Number.isFinite(precomputedCagrRaw)
    ? precomputedCagrRaw
    : null;
...
function computeScenarioOutcome(scenario, { price, eps, horizon }) {
  ...
  const fallbackEpsCagr =
    Number.isFinite(scenario?.growth?.epsCagr) &&
    scenario.growth.epsCagr !== null
...
function computeMetrics(config) {
  ...
  const entryPriceForTarget =
    targetCagr > -1 && price > 0
      ? expectedTerminalPrice / (1 + targetCagr) ** horizon
      : 0;
...
function renderSummary(config) {
  ...
  const benchmarkDescriptor =
    metrics.benchmarkDescriptor || getBenchmarkDescriptor(preferences);
...
function renderScenarioCards(config) {
  ...
  outcomes.forEach((outcome) => {
    const terminalEps =
      outcome && Number.isFinite(outcome.terminalEps)
        ? outcome.terminalEps
        : null;
    const totalEarnings =
  ...
  [
    {
      ...
      value: formatCurrency(metrics.expectedTerminalPrice),
    },
    {
...
function aggregateScenarios(configs, horizon) {
  ...
  return Array.from(scenarioMap.entries()).map(([id, entry]) => {
    ...
    const priceCagr =
      multiple > 0 && horizon > 0 ? multiple ** (1 / horizon) - 1 : 0;
    const normalizedProb = entry.weightedProb;
    const earningsCagr =
      entry.earningsWeight > 0
        ? entry.weightedEarningsCagr / entry.earningsWeight
        : null;
...
function normalizeConfig(raw, holdingDetails = {}) {
  ...
  preferences.overrides = preferences.overrides
  ...
  preferences.horizon = normalizeNumber(
    preferences.horizon ?? legacyManual.horizon,
    5,
  );
  ...
  ["price", "eps", "volatility"].forEach((key) => {
    const value =
      overrides[key] !== undefined ? overrides[key] : legacyManual[key];
  ...
  const holdingShares =
    holdingDetails?.shares ?? legacyManual.shares ?? raw.shares;
  ...
  config.position.constraints = config.position.constraints || {
    minWeight: 0,
    maxWeight: 0.3,
  };
...
async function buildConfigs() {
    ...
    (analysisIndex.tickers || []).map(async (entry) => {
      try {
        ...
        const config = normalizeConfig(
          {
            ...raw,
            symbol: raw.symbol || entry.symbol,
  ...
  const totalValue = validConfigs.reduce(
    (sum, cfg) => sum + cfg.marketValue,
    0,
  );
  ...
  const ordered = portfolioConfig
    ? [portfolioConfig, ...validConfigs]
    : validConfigs;
