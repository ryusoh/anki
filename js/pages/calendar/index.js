...
import {
  ...
  applyCurrencyColors,
} from "@pages/calendar/colorUtils.js";
import {
  ensureEntryDisplay,
...
function attachDateChangeHandler(cal) {
  const dateChangeHandler = async (context) => {
    ...
    logger.log(
      "Encountered domain outside cached range, refreshing calendar data",
    );
...
export function renderLabels(cal, byDate, state, currencySymbols) {
  ...
  const monthState =
    state && state.monthlyPnl instanceof Map ? state : appState;
  /* istanbul ignore next: defensive state handling in render labels */
...
function setupEventListeners(cal, byDate, state, currencySymbols) {
  ...
  document
    .querySelector(CALENDAR_SELECTORS.prevButton)
  ...
  /* istanbul ignore next: event listener registration in test environment */
  document
    .querySelector(CALENDAR_SELECTORS.nextButton)
    .addEventListener("click", (e) => {
      /* istanbul ignore next: event handler execution in test environment */
      e.preventDefault();
      /* istanbul ignore next: event handler execution in test environment */
  ...
  /* istanbul ignore next: event listener registration in test environment */
  document
    .querySelector(CALENDAR_SELECTORS.todayButton)
    .addEventListener("click", (e) => {
      /* istanbul ignore next: event handler execution in test environment */
      e.preventDefault();
      /* istanbul ignore next: event handler execution in test environment */
    ...
    () => {
      ...
      if (
        ...
        const prevButton = document.querySelector(
          CALENDAR_SELECTORS.prevButton,
        );
        /* istanbul ignore next: touch event handling in test environment */
        const nextButton = document.querySelector(
          CALENDAR_SELECTORS.nextButton,
        );
        ...
        if (horizontalDistance > 0) {
          ...
          if (prevButton && !prevButton.disabled) {
            /* istanbul ignore next: touch event handling in test environment */
            logger.log(
              "Touch swipe right detected - navigating to previous month",
            );
            /* istanbul ignore next: touch event handling in test environment */
            state.isCalendarTransition = true;
            state.isAnimating = true;
          ...
          try {
            ...
            calendarInstance
              ...
              .catch((refreshError) => {
                ...
                appState.isCalendarTransition = false;
                logger.error(
                  "Full calendar refresh also failed:",
                  refreshError,
                );
              });
          } catch (refreshError) {
...
export async function initCalendar() {
  try {
    ...
    const { processedData, byDate, rates, monthlyPnl } =
      await getCalendarData(DATA_PATHS);
    appState.rates = rates;
    appState.monthlyPnl = monthlyPnl instanceof Map ? monthlyPnl : new Map();
    ...
    /* istanbul ignore next: defensive data parsing fallback in calendar init */
    const rawLastDate =
      parseDataDate(processedData[processedData.length - 1].date) ||
      firstDataDate;
    /* istanbul ignore next: defensive data parsing fallback in calendar init */
    const lastDataDate = Number.isFinite(rawLastDate.getTime())
      ? rawLastDate
      : firstDataDate;
    ...
    /* istanbul ignore next: calendar range configuration calculation */
    const maxAvailableSpan = Math.max(
      1,
      lastVisibleIndex - firstLabelIndex + 1,
    );
    /* istanbul ignore next: calendar range configuration calculation */
    ...
    const paintConfig = {
      ...
      date: {
        ...
        max: (function () {
          ...
          /* istanbul ignore next: mathematical calculation for max date in calendar config */
          return new Date(
            Math.max(endOfCurrentMonth.getTime(), lastDataDate.getTime()),
          );
        })(),
        highlight: [todayNy],
      },
...
export function autoInitCalendar() {
  if (
    ...
    if (PERLIN_BACKGROUND_SETTINGS?.enabled) {
      perlinBackgroundHandle = mountPerlinPlaneBackground(
        PERLIN_BACKGROUND_SETTINGS,
      );
    }
    initCalendar().then(() => {
