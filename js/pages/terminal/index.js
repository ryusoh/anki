...
function convertCurrencySeries(series, targetCurrency) {
  ...
  return series.map((item) => {
    const result = { ...item };
    if ("value" in item) {
      result.value = convertValueToCurrency(
        item.value,
        item.date,
        targetCurrency,
      );
...
function ensureSyntheticStart(series, { dateKey, valueKey, zeroProps = {} }) {
  if (series && typeof series === "object" && !Array.isArray(series)) {
    ...
    Object.entries(series).forEach(([key, entries]) => {
      result[key] = ensureSyntheticStart(entries, {
        dateKey,
        valueKey,
        zeroProps,
      });
  ...
  const formattedDate =
    rawDate instanceof Date
      ? syntheticDate
      : syntheticDate.toISOString().split("T")[0];
...
function buildFxRateMaps(fxPayload) {
  ...
  Object.entries(fxPayload.rates).forEach(([date, rateMap]) => {
    ...
    Object.entries(rateMap || {}).forEach(([currency, rateValue]) => {
      ...
      const entry = fxRates[normalizedCurrency] || {
        map: new Map(),
        sorted: [],
      };
...
async function loadTransactions() {
  try {
    ...
    const normalizedPortfolioSeriesMap = ensureSyntheticStart(
      portfolioSeriesMap,
      {
        dateKey: "date",
        valueKey: "value",
    ...
    const normalizedStoredCurrency =
      storedCurrency && SUPPORTED_CURRENCIES.includes(storedCurrency)
        ? storedCurrency
        : null;
    const initialCurrency =
      normalizedStoredCurrency || defaultCurrency || "USD";
    ...
    if (
      tableController &&
      typeof tableController.filterAndSort === "function"
    ) {
