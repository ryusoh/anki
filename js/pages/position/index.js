...
function cloneGlassEffectConfig(config) {
  ...
  try {
    ...
    logger.warn(
      "Unable to deep clone glass effect config. Using reference.",
      error,
    );
    return config;
  }
...
function computeGlassOpacity(glassConfig) {
  ...
  const desiredOpacity = isDesktop
    ? responsiveOpacity.desktop
    : responsiveOpacity.mobile;
...
function applyResponsiveGlassOpacity(
  targetConfig = window.pieChartGlassEffect,
) {
  if (!targetConfig) {
    return;
  }
...
async function startApp() {
  try {
    const fxResponse = await fetch(
      "../data/fx_data.json?t=" + new Date().getTime(),
    );
    if (!fxResponse.ok) {
