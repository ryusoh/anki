...
export function renderAsciiTable({
  ...
  const totalWidth =
    widths.reduce((sum, width) => sum + width + 2, 0) + columnCount + 1;
...
export async function getStatsText(currency = "USD") {
  ...
  try {
    ...
    if (statsDataCache) {
      ...
      const rows = [
        [
          "Total Transactions",
          Number(counts.total_transactions || 0).toLocaleString(),
        ],
        ["Buy Orders", Number(counts.buy_orders || 0).toLocaleString()],
        ["Sell Orders", Number(counts.sell_orders || 0).toLocaleString()],
        [
          "Total Buy Amount",
          formatCurrency(values.total_buy_amount || 0, {
            currency: selectedCurrency,
          }),
        ],
        [
          "Total Sell Amount",
          formatCurrency(values.total_sell_amount || 0, {
            currency: selectedCurrency,
          }),
        ],
        [
          "Net Contributions",
          formatCurrency(values.net_contributions || 0, {
            currency: selectedCurrency,
          }),
        ],
        [
          "Realized Gain",
          formatCurrency(values.realized_gain || 0, {
            currency: selectedCurrency,
          }),
        ],
      ];
    ...
    console.warn("Failed to load or process transaction stats JSON:", error);
  ...
  try {
    ...
    console.warn("Failed to load transaction stats fallback text:", error);
    return "Error loading transaction stats.";
  }
}
...
export async function getHoldingsText(currency = "USD") {
  ...
  try {
    ...
    console.warn("Failed to load or process holdings summary JSON:", error);
  ...
  try {
    ...
    console.warn("Failed to load holdings summary fallback text:", error);
    return "Error loading holdings data.";
  }
}
...
export async function getFinancialStatsText() {
  try {
      ...
      tickers.map(async (entry) => {
        try {
          ...
          return [
            ...
            formatPercentageValue(market.dividendYield, {
              digits: 2,
              mode: "percent",
            }),
          ...
          console.warn("Failed to process financial analysis metric:", error);
          return null;
        }
      }),
    );
    ...
    console.warn("Failed to load or process financial analysis data:", error);
...
export async function getTechnicalStatsText() {
  try {
      ...
      tickers.map(async (entry) => {
        try {
          ...
          return [
            ...
            formatPercentageValue(market.volatility, {
              digits: 2,
              mode: "fraction",
            }),
          ];
        } catch (error) {
          console.warn("Failed to process technical analysis metric:", error);
          return null;
        }
      }),
    );
    ...
    console.warn("Failed to load or process technical analysis data:", error);
...
export async function getCagrText() {
  try {
    ...
    console.warn("Failed to load CAGR data:", error);
    return "Error loading CAGR data.";
  }
}
...
export async function getAnnualReturnText() {
  try {
    ...
    console.warn("Failed to load annual returns data:", error);
    return "Error loading annual returns.";
  }
}
...
export async function getRatioText() {
  try {
    ...
    console.warn("Failed to load Sharpe and Sortino ratios:", error);
    return "Error loading Sharpe and Sortino ratios.";
  }
}
...
export async function getDurationStatsText() {
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    weightedDaysSum += entry.weight * entry.avgAgeDays;
  }
  const weightedAvgDays =
    totalWeight > 0 ? weightedDaysSum / totalWeight : null;
  ...
  let weightedClosedDaysSum = 0;
  if (hasClosedData) {
    for (let i = 0; i < closedSales.length; i++) {
      const item = closedSales[i];
      const qty = Number(item.qty) || 0;
      totalClosedQty += qty;
      weightedClosedDaysSum += qty * (Number(item.days) || 0);
    }
  }
  const weightedClosedAvgDays =
    totalClosedQty > 0 ? weightedClosedDaysSum / totalClosedQty : null;
...
export async function getLifespanStatsText() {
  ...
  const { lotsByTicker, currentPeriodStart, closedPeriods } =
    buildLotSnapshots();
  const hasOpenData =
    currentPeriodStart instanceof Map ? currentPeriodStart.size > 0 : false;
  const hasClosedData =
    Array.isArray(closedPeriods) && closedPeriods.length > 0;
  if (!hasOpenData && !hasClosedData) {
  ...
  const closedEntries = (Array.isArray(closedPeriods) ? closedPeriods : [])
    .filter(
      (period) => !openTickerSet.has(normalizeTickerKey(period.ticker || "")),
    )
    .map((period) => ({
    ...
    .filter(
      (entry) => Number.isFinite(entry.spanDays) && entry.spanDays !== null,
    )
    .sort((a, b) => b.spanDays - a.spanDays);
...
function computeWeightedMedian(entries, weightGetter, valueGetter) {
  // Bolt: Replace .map().filter().reduce() chains with a single O(N) loop
  // to avoid intermediate array allocations and multiple iteration passes.
  const normalized = [];
  let totalWeight = 0;
  for (let i = 0; i < entries.length; i++) {
    const entry = entries[i];
...
function buildLotSnapshots() {
  ...
  sorted.forEach((transaction) => {
    const rawSymbol =
      typeof transaction.security === "string" ? transaction.security : "";
    const orderType =
      typeof transaction.orderType === "string" ? transaction.orderType : "";
    ...
    const adjustedQty =
      quantity * (Number.isFinite(adjustment) ? adjustment : 1);
    ...
    if (normalizedOrderType === "buy") {
      ...
      while (remaining > 0 && lots.length > 0) {
        ...
        const holdingDays =
          lot.date && tradeDate
            ? Math.max(0, (tradeDate - lot.date) / MS_IN_DAY)
            : null;
