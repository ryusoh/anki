...
function createTimeInterpolator(points) {
  ...
  return (time) => {
    ...
    const ratio =
      (clampedTime - leftPoint.time) /
      (rightPoint.time - leftPoint.time || 1e-12);
...
export function buildFxChartSeries(baseCurrency) {
  ...
  const normalizedBase =
    typeof baseCurrency === "string" ? baseCurrency.toUpperCase() : "USD";
  const baseEntry = fxRates[normalizedBase];
  const seriesList = [];

  FX_CURRENCY_ORDER.filter((currency) => currency !== normalizedBase).forEach(
    (currency) => {
...
function drawCrosshairOverlay(ctx, layout) {
  ...
  const hasHover =
    crosshairState.active && Number.isFinite(crosshairState.hoverTime);
  const hasRange =
  ...
  const isCompositionLayout =
    layout.key === "composition" || layout.key === "compositionAbs";
  ...
  if (isCompositionLayout) {
    ...
    const selectedCurrency =
      layout.currency || transactionState.selectedCurrency || "USD";
    ...
    const topHoldings = nonZeroHoldings
    ...
    const boundsHeight = layout.chartBounds
      ...
      : 0;
    const stackMaxValue =
      layout && Number.isFinite(layout.stackMaxValue)
        ? layout.stackMaxValue
        : 100;
    ...
    for (const series of layout.series) {
      ...
      if (
        ...
        const clampedValue = Math.max(
          0,
          Math.min(stackMaxValue, pointerStackValue),
        );
        ...
        if (
          clampedValue >= lowerBound - epsilon &&
          clampedValue <= upperBound + epsilon
        ) {
          const absoluteValue = isAbsoluteMode
            ? componentValue
            : convertValueToCurrency(
...
function drawCompositionHoverPanel(
  ctx,
  layout,
  crosshairX,
  crosshairY,
  ...
  const paddingX = isMobile
    ? (paddingConfig.mobile?.x ?? 10)
    : (paddingConfig.desktop?.x ?? 12);
  const paddingY = isMobile
    ? (paddingConfig.mobile?.y ?? 8)
    : (paddingConfig.desktop?.y ?? 10);
  const lineGapConfig = CROSSHAIR_SETTINGS.compositionHoverLineGap || {};
  const lineGap = isMobile
    ? (lineGapConfig.mobile ?? 4)
    : (lineGapConfig.desktop ?? 6);
  const dotRadius = 4;
  const dotGap = 6;
  ...
  const percentText =
    holding.formattedPercent || `${holding.percent?.toFixed?.(2) ?? 0}%`;

  ctx.font = `${lineFontSize}px ${fontFamily}`;
  let absoluteText = holding.formattedValue || null;
  if (!absoluteText || !absoluteText.trim()) {
    const rawValue = Number.isFinite(holding.absoluteValue)
      ? holding.absoluteValue
      : null;
  ...
  const preferRight =
    crosshairX < bounds.left + (bounds.right - bounds.left) / 2;
  ...
  const backgroundColor =
    CROSSHAIR_SETTINGS.compositionHoverBackground || "rgba(6, 9, 22, 0.8)";
  const borderColor =
    CROSSHAIR_SETTINGS.compositionHoverBorder || "rgba(255,255,255,0.08)";
...
function requestChartRedraw() {
  if (
    crosshairChartManager &&
    typeof crosshairChartManager.redraw === "function"
  ) {
...
export function buildContributionSeriesFromTransactions(
  ...
  // ⚡ Bolt: Replace .map().sort().map() chain with a single pre-allocated array loop
  ...
  for (let i = 0; i < len; i++) {
    ...
    sortedTransactions[i] = { t, parsedDate: new Date(t.tradeDate).getTime() };
  }
  ...
  if (lastPoint) {
    ...
    const targetDate = Number.isNaN(targetDateRaw.getTime())
      ? today
      : targetDateRaw;
  ...
  if (includeSyntheticStart && series.length > 0) {
    ...
    const firstActual =
      series.find(
        (point) => (point.orderType || "").toLowerCase() !== "padding",
      ) || series[0];
    ...
    if (!Number.isNaN(firstDate.getTime()) && Math.abs(firstValue) > epsilon) {
      ...
      const existing = series.find(
        (point) => point.tradeDate === syntheticDateStr,
      );
      if (!existing) {
        series.unshift({
  ...
  return series.map((point) => {
    ...
    const convertedNet = convertValueToCurrency(
      point.netAmount,
      dateRef,
      selectedCurrency,
    );
...
export function buildFilteredBalanceSeries(
  transactions,
  historicalPrices,
  splitHistory,
) {
  ...
  // ⚡ Bolt: Replace .map().sort().map() chain with a single pre-allocated array loop
  // and in-place .sort() to minimize intermediate object allocations and GC pressure.
  const len2 = transactions.length;
  const sortedTransactions = new Array(len2);
  ...
  if (
    ...
    Number.isNaN(lastTransactionDate.getTime())
  ) {
    return [];
  }
  ...
  (Array.isArray(splitHistory) ? splitHistory : []).forEach((split) => {
    ...
    const multiplier =
      Number(split.splitMultiplier) || Number(split.split_multiplier) || 1;
  ...
  while (iterDate <= lastDate) {
    ...
    holdings.forEach((qty, symbol) => {
      ...
      const price = getPriceFromHistoricalData(
        historicalPrices,
        symbol,
        dateStr,
      );
...
function injectSyntheticStartPoint(
  filteredData,
  fullSeries,
  filterFrom = null,
) {
...
function drawMountainFill(ctx, coords, baselineY, options) {
  ...
  clampedBaselineY = Math.min(
    Math.max(clampedBaselineY, bounds.top),
    bounds.bottom,
  );

  const areaCoords = (
    coords.length === 1 ? [coords[0], coords[0]] : coords
  ).map((coord) => ({
    x: coord.x,
    y: coord.y,
  }));
  ...
  offCtx.lineTo(
    areaCoords[areaCoords.length - 1].x - bounds.left,
    clampedBaselineY - bounds.top,
  );
  ...
  const alphaGradient = offCtx.createLinearGradient(
    0,
    gradientTop,
    0,
    gradientBottom,
...
function getSmoothingConfig(chartType) {
  ...
  const methodConfig =
    CHART_SMOOTHING.methods[methodName] || CHART_SMOOTHING.methods.balanced;
...
function getChartColors(rootStyles) {
  return {
    portfolio:
      rootStyles.getPropertyValue("--portfolio-line").trim() || "#7a7a7a",
    contribution:
      rootStyles.getPropertyValue("--contribution-line").trim() || "#b3b3b3",
...
function drawMarker(context, x, y, radius, isBuy, colors, chartBounds) {
  ...
  const clampedY = Math.max(
    chartBounds.top + radius,
    Math.min(y, chartBounds.bottom - radius),
  );
...
function drawEndValue(
  ...
  textX = Math.max(
    padding.left + 2,
    Math.min(textX, padding.left + plotWidth - textWidth - 2),
  );
  textY = Math.max(
    padding.top + textHeight / 2,
...
function generateConcreteTicks(
  yMin,
  yMax,
  isPerformanceChart,
  currency = "USD",
  ...
  for (
    let tick = niceMin;
    tick <= niceMax + tickSpacing * 0.5;
    tick += tickSpacing
  ) {
    const rounded = Number(
      (Math.round(tick / tickSpacing) * tickSpacing).toFixed(6),
    );
    ticks.push(rounded);
  }
  ...
  return ticks.filter(
    (tick) =>
      tick >= yMin - tickSpacing * 0.25 && tick <= yMax + tickSpacing * 0.25,
  );
}
...
function generateYearBasedTicks(minTime, maxTime) {
  ...
  const isSingleYear =
    startDate.getFullYear() === endDate.getFullYear() || dataSpanMonths <= 15;
  ...
  for (let i = 0; i < ticks.length; i++) {
    ...
    const isTooClose = filteredTicks.some(
      (existingTick) =>
        Math.abs(currentTick.time - existingTick.time) <
    ...
    if (!isTooClose) {
      ...
      const existingIsYearBoundary = filteredTicks.some(
        (existingTick) =>
          Math.abs(currentTick.time - existingTick.time) <
            10 * 24 * 60 * 60 * 1000 && existingTick.isYearStart,
      );
...
function drawAxes(
  ...
  const ticks = generateConcreteTicks(
    yMin,
    yMax,
    isPerformanceChart || forcePercent,
    currency,
  );
...
async function drawContributionChart(ctx, chartManager, timestamp) {
  ...
  const runningAmountSeries = Array.isArray(
  ...
  const portfolioSeries = Array.isArray(transactionState.portfolioSeries)
    ...
    : [];
  const filteredTransactions = Array.isArray(
  ...
  if (contributionTransactions.length > 0) {
    ...
    contributionSource = getContributionSeriesForTransactions(
    ...
    contributionFromTransactions =
      filtersActive &&
      Array.isArray(contributionSource) &&
      contributionSource.length > 0;
    if (!filtersActive && contributionSource !== runningAmountSeries) {
      setRunningAmountSeries(contributionSource);
    }
  } else {
    const mappedSeries =
      transactionState.runningAmountSeriesByCurrency?.[selectedCurrency] ||
      null;
  ...
  if (
    filtersActive &&
    (!historicalPrices || Object.keys(historicalPrices).length === 0)
  ) {
    try {
  ...
  const hasBalanceSeries =
    Array.isArray(balanceSource) && balanceSource.length > 0;
  ...
  const filterFrom = chartDateRange.from
    ? parseLocalDate(chartDateRange.from)
    : null;
  const filterTo = chartDateRange.to ? parseLocalDate(chartDateRange.to) : null;
  const filterFromTime =
    filterFrom && Number.isFinite(filterFrom.getTime())
      ? filterFrom.getTime()
      : null;
  const filterToTime =
  ...
  const filterDataByDateRange = (data) => {
    return data.filter((item) => {
      ...
      const filterFromStr = filterFrom
        ? filterFrom.toISOString().split("T")[0]
        : null;
      const filterToStr = filterTo
        ? filterTo.toISOString().split("T")[0]
        : null;
      ...
      const isPadding =
        item.orderType && item.orderType.toLowerCase() === "padding";
  ...
  const balanceSmoothingConfig =
    getSmoothingConfig("balance") || contributionSmoothingConfig;
  ...
  const shouldSmoothBalance =
    !filtersActive &&
    balanceDataWithinRange.length > 2 &&
    balanceSmoothingConfig;
  const balanceData = shouldSmoothBalance
    ? smoothFinancialData(
        balanceDataWithinRange.map((item) => ({
          x: item.date.getTime(),
          y: item.value,
        })),
        balanceSmoothingConfig,
  ...
  const volumeTop =
    padding.top + plotHeight + (volumeHeight > 0 ? volumeGap : 0);
  ...
  let minTime =
    effectiveMinTimes.length > 0
      ? Math.min(...effectiveMinTimes)
      : fallbackMinTime;
  ...
  const combinedValues = [...contributionValues, ...balanceValues].filter(
  ...
  const xScale = (t) =>
    ...
    (maxTime === minTime
      ? plotWidth / 2
      : ((t - minTime) / (maxTime - minTime)) * plotWidth);
  const yScale = (v) =>
  ...
  const filterStartTime = Number.isFinite(filterFromTime)
    ? filterFromTime
    : null;

  const formatBalanceValue = (value) =>
    formatCurrencyCompact(value, {
      currency: transactionState.selectedCurrency || "USD",
    });
  ...
  if (volumeHeight > 0) {
    ...
    volumeYScale = (value) =>
      volumePadding.top +
      volumeHeight -
      ((value - volumeYMin) / volumeRange) * volumeHeight;
  ...
  if (showChartLabels && showContribution && rawContributionData.length > 0) {
    ...
    const lastContribution =
      rawContributionData[rawContributionData.length - 1];
  ...
  if (showChartLabels && showBalance && rawBalanceData.length > 0) {
    ...
    const balanceStartColor = balanceGradient
      ? balanceGradient[0]
      : colors.portfolio;
    const balanceEndColor = balanceGradient
      ? balanceGradient[1]
      : colors.portfolio;
    ...
    if (firstContributionLabelY !== null) {
      ...
      if (Math.abs(firstBalanceY - firstContributionLabelY) < minGap) {
        if (firstBalanceY >= firstContributionLabelY) {
          ...
          firstBalanceY = Math.max(
            firstBalanceY - minGap,
            padding.top + minGap / 2,
          );
        }
      }
    }
  ...
  const baseSeries = animatedSeries.map((series) => {
    ...
    if (series.key === "balance") {
      displayColor =
        (BALANCE_GRADIENTS.balance && BALANCE_GRADIENTS.balance[1]) ||
        colors.portfolio;
  ...
  chartLayouts.contribution = {
    ...
    invertX: (pixelX) => {
      ...
      const clampedX = Math.max(
        padding.left,
        Math.min(padding.left + plotWidth, pixelX),
      );
...
async function drawPerformanceChart(ctx, chartManager, timestamp) {
  const performanceSeries =
    ...
    typeof transactionState.performanceSeries === "object"
      ? transactionState.performanceSeries
      : {};
  const selectedCurrency = transactionState.selectedCurrency || "USD";
  ...
  const allPossibleSeries = orderedKeys.map((key) => {
    const points = Array.isArray(performanceSeries[key])
      ? performanceSeries[key]
      : [];
  ...
  const visibility = chartVisibility || {};
  const seriesToDraw = allPossibleSeries.filter(
    (s) => visibility[s.key] !== false,
  );
  ...
  chartLayouts.performance = {
    ...
    series: renderedSeries.map((series) => ({
      ...
      formatDelta: (delta, percent) =>
        formatPercentInline(
          percent !== null && percent !== undefined ? percent : delta,
        ),
    })),
  };
...
function drawFxChart(ctx, chartManager, timestamp) {
  ...
  filteredSeries.forEach((series) => {
    ...
    const gradientStops =
      FX_GRADIENTS[series.quote] || FX_GRADIENTS[series.label] || null;
    const darkColor = gradientStops
      ? gradientStops[0]
      : darkenColor(baseColor, 0.15);
    const lightColor = gradientStops
      ? gradientStops[1]
    ...
    renderedSeries.push({
      ...
      color: resolvedColor,
      points: coords.map((coord) => ({ time: coord.time, value: coord.value })),
      rawPoints: coords.map((coord) => ({
        time: coord.time,
        value: coord.rawValue,
      })),
    });
...
function buildCompositionDisplayOrder(
  ...
  const selectedOrder = baseOrder.filter((ticker) =>
    filterSet.has(ticker.toUpperCase()),
  );
  if (selectedOrder.length === 0) {
    return { order: [...baseOrder], filteredOthers: null };
  }

  const remainder = baseOrder.filter(
    (ticker) => !filterSet.has(ticker.toUpperCase()),
  );
  const includeFilteredOthers =
    remainder.length > 0 && !filterSet.has("OTHERS");
...
function renderCompositionChartWithMode(ctx, chartManager, data, options = {}) {
  ...
  const dates =
    filteredIndices.length > 0
      ? filteredIndices.map((i) => rawDates[i])
      : rawDates.slice();
  ...
  const percentOthersSeries =
    valueMode === "absolute" ? filteredReference : filteredOthers;
  const activeTickerOrder =
    filteredOrder.length > 0 ? filteredOrder : baseTickerOrder;
  ...
  activeTickerOrder.forEach((ticker, tickerIndex) => {
    const values =
      ticker === "Others" && usingFilteredOthers
        ? filteredOthers
        : chartData[ticker] || [];
    ...
    const color =
      resolveTickerColor(ticker) || colors[tickerIndex % colors.length];
    ctx.beginPath();
  ...
  const othersPercentage =
    othersPercentSeries.length > 0
      ? (othersPercentSeries[latestIndex] ?? 0)
      : 0;
...
function drawCompositionChartLoader(ctx, chartManager, valueMode) {
  ...
  if (compositionDataCache) {
    renderCompositionChartWithMode(ctx, chartManager, compositionDataCache, {
      valueMode,
    });
    return;
  }
