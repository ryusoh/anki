...
function getFxSnapshotLine() {
  ...
  seriesList.forEach((series) => {
    ...
    snapshots.push(
      `${baseCurrency}/${series.quote} ${formatFxInline(latestPoint.value)}`,
    );
  });
  if (!snapshots.length) {
    return null;
...
function getPerformanceSnapshotLine({ includeHidden = false } = {}) {
  ...
  orderedKeys.forEach((key) => {
    ...
    const rawPoints = Array.isArray(performanceSeries[key])
...
async function getCompositionSnapshotLine({
  ...
  if (Array.isArray(filterTickers) && filterTickers.length > 0) {
    ...
    const selected = holdings.filter((holding) =>
      normalized.has(holding.ticker.toUpperCase()),
    );
  ...
  const formatted = displayHoldings
    .filter(
      (holding) => Number.isFinite(holding.percent) && holding.percent > 0.1,
    )
    .map((holding) => {
...
export function updateTerminalCrosshair(snapshot, rangeSummary) {
  ...
  requestAnimationFrame(() =>
    overlay.classList.add("terminal-crosshair-active"),
  );
...
async function buildContributionChartSummary(
  dateRange = transactionState.chartDateRange,
) {
  ...
  const balanceSummary = computeSeriesSummary(
    balanceSource,
    dateRange,
    "date",
    "value",
...
async function getContributionSummaryText(
  dateRange = transactionState.chartDateRange,
) {
  const { contributionSummary, balanceSummary } =
    await buildContributionChartSummary(dateRange);
  const blocks = [
...
export function initTerminal({
  ...
  function autocompleteCommand(input) {
    ...
    if (searchPrefix.includes(" ")) {
      ...
      if (parts.length >= 2 && (parts[0] === "stats" || parts[0] === "s")) {
        ...
        matches = normalizedSubPrefix
          ? PLOT_SUBCOMMANDS.filter((cmd) =>
              cmd.startsWith(normalizedSubPrefix),
            )
          : PLOT_SUBCOMMANDS;
      } else if (
        parts.length >= 2 &&
        (parts[0] === "help" || parts[0] === "h")
      ) {
        const subPrefix = parts[1] ? parts[1].toLowerCase() : "";
        matches = subPrefix
    ...
    if (searchPrefix.includes(" ") && searchPrefix.split(" ")[0] === "stats") {
      ...
      searchPrefix.includes(" ") &&
      searchPrefix.split(" ")[0] === "s"
    ) {
      const baseCommand = "s ";
      input.value = baseCommand + completed + (shouldAppendSpace ? " " : "");
    } else if (
      searchPrefix.includes(" ") &&
      searchPrefix.split(" ")[0] === "plot"
    ) {
      const baseCommand = "plot ";
      input.value = baseCommand + completed + (shouldAppendSpace ? " " : "");
    } else if (
      searchPrefix.includes(" ") &&
      searchPrefix.split(" ")[0] === "p"
    ) {
      const baseCommand = "p ";
      input.value = baseCommand + completed + (shouldAppendSpace ? " " : "");
    } else if (
      searchPrefix.includes(" ") &&
      searchPrefix.split(" ")[0] === "help"
    ) {
      const baseCommand = "help ";
      input.value = baseCommand + completed + (shouldAppendSpace ? " " : "");
    } else if (
      searchPrefix.includes(" ") &&
      searchPrefix.split(" ")[0] === "h"
    ) {
      const baseCommand = "h ";
      input.value = baseCommand + completed + (shouldAppendSpace ? " " : "");
    } else {
  ...
  async function processCommand(command) {
    ...
    switch (cmd.toLowerCase()) {
      ...
      case "reset":
        ...
        const resetTableContainer = document.querySelector(
          ".table-responsive-container",
        );
        const resetPlotSection = document.getElementById(
          "runningAmountSection",
        );
        const resetPerformanceSection =
      ...
      case "clear":
        ...
        document
          .querySelectorAll(
            ".table-responsive-container, #runningAmountSection",
          )
      ...
      case "plot":
        if (args.length === 0) {
          ...
          switch (subcommand) {
            ...
            case "performance":
              dateRange = applyDateArgs(rawArgs);
              const perfSection = document.getElementById(
                "runningAmountSection",
              );
            ...
            case "compositionabsolute": {
              ...
              const compSection = document.getElementById(
                "runningAmountSection",
              );
  ...
  function isTransactionTableVisible() {
    ...
    return Boolean(
      tableContainer && !tableContainer.classList.contains("is-hidden"),
    );
  }
  ...
  async function handleTerminalInput(e) {
    ...
    switch (e.key) {
      ...
      case "ArrowUp":
        e.preventDefault();
        if (
          transactionState.historyIndex <
          transactionState.commandHistory.length - 1
        ) {
          setHistoryIndex(transactionState.historyIndex + 1);
          input.value =
            transactionState.commandHistory[transactionState.historyIndex];
        }
        resetAutocompleteState();
        requestFadeUpdate();
        break;
  ...
  document.addEventListener("transactionFilterResult", (event) => {
    ...
    const { count } = detail;
    const searchTerm =
      typeof detail.searchTerm === "string" ? detail.searchTerm.trim() : "";
    if (count === 0 && searchTerm) {
      if (lastEmptyFilterTerm !== searchTerm) {
        appendMessage(
          "No transactions match the current filter. Type 'clear' to reset.",
        );
...
function parseDateRange(args) {
  ...
  if (args.length === 1) {
    ...
    if (quarterToken) {
      const resolved = resolveQuarterRange(
        quarterToken.year,
        quarterToken.quarter,
        "full",
      );
      debugContext("parseDateRange:single-quarter", {
        arg,
        defaultYear,
        resolved,
      });
    ...
    if (quarterToken) {
      ...
      debugContext("parseDateRange:from-quarter", {
        arg,
        defaultYear,
        resolved,
      });
