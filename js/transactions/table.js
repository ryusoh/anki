...
function parseCommandPalette(value) {
  ...
  tokens.forEach((token) => {
    ...
    switch (key.toLowerCase()) {
      case "type":
        commands.type =
          val.toLowerCase() === "buy" || val.toLowerCase() === "sell"
            ? val
            : null;
        break;
      case "security":
...
function createFilterDropdown(column) {
  ...
  const options =
    column === "orderType"
      ...
      : [
          "All",
          ...new Set(transactionState.allTransactions.map((t) => t.security)),
        ].sort();
  ...
  options.forEach((option) => {
    ...
    const applyFilter = (e) => {
      ...
      const command =
        option === "All"
          ? ""
          : `${column === "orderType" ? "type" : "security"}:${option}`;
      const terminalInput = document.getElementById("terminalInput");
      if (terminalInput) {
...
export function initTable({ onFilterChange } = {}) {
  filterChangeListener =
    typeof onFilterChange === "function" ? onFilterChange : null;
  setupTableControls();
  return {
    filterAndSort,
    closeAllFilterDropdowns,
