...
export function setAllTransactions(transactions) {
  transactionState.allTransactions = Array.isArray(transactions)
    ? transactions
    : [];
}

export function setFilteredTransactions(transactions) {
  transactionState.filteredTransactions = Array.isArray(transactions)
    ? transactions
    : [];
}
...
export function setSelectedCurrency(currency) {
  ...
  transactionState.currencySymbol =
    CURRENCY_SYMBOLS[normalized] || DEFAULT_SYMBOL;
}
...
export function setFxRatesByCurrency(fxMap) {
  transactionState.fxRatesByCurrency =
    fxMap && typeof fxMap === "object" ? fxMap : {};
}
