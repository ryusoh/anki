...
export async function loadPortfolioSeries() {
  try {
    ...
    if (!response.ok) {
      logger.warn(
        "balance_series.json not found; portfolio balance line disabled",
      );
      return [];
    }
    ...
    const normalizeSeries = (entries) =>
      (Array.isArray(entries) ? entries : [])
        ...
        .filter(
          (entry) =>
            typeof entry.date === "string" && Number.isFinite(entry.value),
        );
  ...
  try {
    ...
    const normalizeSeries = (entries) =>
      (Array.isArray(entries) ? entries : [])
        .map((entry) => ({
          tradeDate:
            typeof entry.tradeDate === "string" ? entry.tradeDate : null,
          amount: Number(entry.amount),
          orderType:
            typeof entry.orderType === "string" ? entry.orderType : "padding",
...
export async function loadPerformanceSeries() {
  try {
    ...
    if (!response.ok) {
      logger.warn(
        "performance_series.json not found; performance chart disabled",
      );
      return {};
    }
    ...
    Object.entries(payload).forEach(([rawKey, points]) => {
      ...
      const normalizedPoints = points
        ...
        .filter(
          (point) =>
            typeof point.date === "string" && Number.isFinite(point.value),
        );
...
export async function loadFxDailyRates() {
  try {
    ...
    if (!response.ok) {
      logger.warn(
        "fx_daily_rates.json not found; currency conversion disabled",
      );
      return null;
    }
    const payload = await response.json();
