...
function ensureFrostedFilter(d3Instance) {
  ...
  if (filter.empty()) {
    ...
    filter
      ...
      .attr(
        "stdDeviation",
        CALENDAR_MONTH_LABEL_BACKGROUND.blurStdDeviation || 8,
      );
    /* istanbul ignore next: defensive programming for missing SVG */
    filter
...
export function updateMonthLabels(d3Instance, state, currencySymbols) {
  ...
  const viewportThreshold =
    CALENDAR_MONTH_LABEL_BACKGROUND.maxWidth ?? UI_BREAKPOINTS.MOBILE;
  /* istanbul ignore next: defensive programming for UI state calculation */
  ...
  /* istanbul ignore next: defensive programming for UI state calculation */
  const classList =
    pageWrapper && typeof pageWrapper.classList === "object"
      ? pageWrapper.classList
      : null;
  /* istanbul ignore next: defensive programming for UI state calculation */
  ...
  const shouldShowBackground =
    backgroundsEnabled &&
    (isMobileViewport || (!isMobileViewport && !isZoomed));
  /* istanbul ignore next: defensive programming for UI state calculation */
  const filterId = shouldShowBackground
    ? ensureFrostedFilter(d3Instance)
    : null;
  /* istanbul ignore next: defensive programming for UI state calculation */
  const transitionDuration =
    CALENDAR_MONTH_LABEL_BACKGROUND.transitionDuration ?? 200;
  ...
  const intervalMs =
    Number.isFinite(intervalValue) && intervalValue > 0 ? intervalValue : 140;
  const waveSizeValue = Number(highlightConfig.waveSize);
  const waveSize = Math.max(
    1,
    Number.isFinite(waveSizeValue) ? Math.floor(waveSizeValue) : 2,
  );
  const baseWaveColor =
  ...
  const pnlLightenFactor =
    Number.isFinite(lightenFactorValue) &&
    lightenFactorValue >= 0 &&
    lightenFactorValue <= 1
      ? lightenFactorValue
      : 0.55;
  ...
  selection.each(function handleDomainLabel() {
    ...
    const previousBaseSpan =
      el && typeof el.select === "function"
        ? el.select("tspan.domain-label-base")
        : null;
    if (
      previousBaseSpan &&
    ...
    const baseText =
      typeof baseAttrValue === "string" && baseAttrValue.trim()
        ? baseAttrValue
        : currentText;
      ...
      if (shouldShowBackground) {
        ...
        if (backgroundRect.empty()) {
          backgroundRect = parent
            ...
            .attr(
              "stroke-width",
              CALENDAR_MONTH_LABEL_BACKGROUND.strokeWidth || 0,
            )
            .attr("opacity", 0)
    ...
    const info =
      monthKey && state.monthlyPnl instanceof Map
        ? state.monthlyPnl.get(monthKey)
        : null;
    const highlightMonthKey =
      typeof state.highlightMonthKey === "string"
        ? state.highlightMonthKey
        : null;
    const shouldHighlightMonth = Boolean(
      highlightMonthKey && monthKey === highlightMonthKey,
    );
    let color = null;
    if (info) {
      const currency = state.selectedCurrency || "USD";
    ...
    const openSpan = el
      .append("tspan")
      .attr("class", "domain-label-open-bracket")
      .text(" (");
    const percentSpan = el
      .append("tspan")
      .attr("class", "domain-label-percent")
      .text(percentText ?? "");
    const separatorSpan = el
      .append("tspan")
      .attr("class", "domain-label-separator")
      .text(", ");
    const changeSpan = el
      .append("tspan")
    ...
    const closeSpan = el
      .append("tspan")
      .attr("class", "domain-label-close-bracket")
      .text(")");
