 ...
 */
export function setHighlightState(
  target,
  shouldHighlight,
 ...
 */
export function setCompositeHighlight(
  textTarget,
  backgroundTarget,
  shouldHighlight,
) {
  setHighlightState(textTarget, shouldHighlight, { className: "text-shimmer" });
  setHighlightState(backgroundTarget, shouldHighlight, {
    className: "text-shimmer-bg",
  });
}
...
function startThinking(node, options) {
  ...
  const charNodes = createCharacterNodes(
    node,
    baseText,
    mergedOptions.baseColor,
  );
  if (!charNodes.length) {
    return;
  }
  ...
  const waveSize = Math.max(
    1,
    Math.min(mergedOptions.waveSize || 1, charNodes.length),
  );
  ...
  const disableAnimation = Boolean(
    mergedOptions.disableAnimation || prefersReducedMotion(),
  );
  applyWave();
