...
function ensureToggleElements() {
  ...
  if (
    !iconsDecorated &&
    Array.isArray(currencyButtons) &&
    currencyButtons.length
  ) {
    applyCurrencyIcons();
    iconsDecorated = true;
  }
...
function applyCurrencyIcons() {
  currencyButtons.forEach((button) => {
    ...
    const resolvedClasses = Array.isArray(iconClasses)
      ? iconClasses
      : [iconClasses];
...
function activateCurrency(currency, { emit = true, persist = true } = {}) {
  ...
  const hasButtons =
    Array.isArray(currencyButtons) && currencyButtons.length > 0;
...
export function initCurrencyToggle() {
  ...
  let initialCurrency =
    normalizeCurrency(
      currencyButtons.find((button) => button.classList.contains("active"))
        ?.dataset.currency,
    ) || "USD";
  ...
  if (
    storedCurrency &&
    currencyButtons.some(
      (btn) => normalizeCurrency(btn.dataset.currency) === storedCurrency,
    )
  ) {
...
export function cycleCurrency(step) {
  ...
  let currentIndex = currencyButtons.findIndex((btn) =>
