...
GENDERS = [('m', 'Male'), ('f', 'Female'), ('any', 'Any')]
...
class Forvo(Service):
    ...
    def get_language_code_from_forvo_lang(self, forvo_lang):
        forvo_language_id_map = {'zh': 'zh_cn', 'ind': 'id_', 'pt': 'pt_pt'}
    ...
    def run(self, text, options, path):
        ...
        if self.languagetools.use_plus_mode():
            ...
            service = 'Forvo'
            voice_key = {'language_code': forvo_language_code, 'country_code': country_code}
            if sex != 'any':
                voice_key['gender'] = sex
            if preferred_user != PREFERRED_USER_DEFAULT_KEY:
                voice_key['preferred_user'] = preferred_user

            languagetools_language_code = self.get_language_code_from_forvo_lang(
                forvo_language_code
            )
            self.languagetools.generate_audio_v2(
                text, service, 'batch', languagetools_language_code, 'n/a', voice_key, {}, path
            )
        else:
