...
class Azure(Service):
    ...
    def options(self):
        ...
        result = [
            ...
            dict(key='azurespeed', label='Speed', values=(0.5, 3.0), default=1.0, transform=float),
            dict(key='azurepitch', label='Pitch', values=(-100, 100), default=0, transform=int),
        ]
...
<voice name="{voice_name}"><prosody rate="{rate:0.1f}" pitch="{pitch:+.0f}Hz" >{text}</prosody></voice>
            ...
            if response.status_code == 200:
                ...
                error_message = (
                    f"Status code: {response.status_code} reason: {response.reason} voice: [{voice_name}] language: [{language} "
