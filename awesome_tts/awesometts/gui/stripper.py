...
class BrowserStripper(Dialog):
    ...
    def _ui(self):
        ...
        for value, label in [
            ...
            (
                'any',
                "&all [sound] tags, regardless of origin, and paths " "generated by AwesomeTTS",
            ),
        ]:
    ...
    def show(self, *args, **kwargs):
        ...
        self.findChild(Note, 'intro').setText(
            ...
            "fields:" % (len(self._notes), "s" if len(self._notes) != 1 else "")
        )

        layout = aqt.qt.QVBoxLayout()
        for field in sorted({field for note in self._notes for field in note.keys()}):
    ...
    def _process_notes(self, fields, mode, stat):
        ...
        for note in self._notes:
            ...
            for field in fields:
                ...
                new_value = (
                    strips.ours(old_value)
                    if mode == 'ours'
                    else strips.theirs(old_value) if mode == 'theirs' else strips.univ(old_value)
                )

                if old_value == new_value:
                    self._addon.logger.debug(
                        "Note %d unchanged for %s\n%s", note.id, field, old_value
                    )
                else:
                    self._addon.logger.info(
                        "Note %d upd for %s\n%s\n%s", note.id, field, old_value, new_value
                    )
    ...
    def _build_messages(self, stat):
        ...
        if stat['fields']['skip'] == 1:
            ...
            messages.append("\n%d fields were not present on some notes." % stat['fields']['skip'])
