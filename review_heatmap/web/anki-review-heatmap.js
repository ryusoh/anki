          ...
          x: function (d2) {
            if (self.options.domain === "month") {
              if (self.options.colLimit > 0 || self.options.rowLimit > 0) {
                return Math.floor(
                  (d2.getHours() + (d2.getDate() - 1) * 24) /
                    self._domainType.hour.row(d2),
                );
              }
              return (
                Math.floor(d2.getHours() / self._domainType.hour.row(d2)) +
                (d2.getDate() - 1) * 4
              );
            } else if (self.options.domain === "week") {
              if (self.options.colLimit > 0 || self.options.rowLimit > 0) {
                return Math.floor(
                  (d2.getHours() + self.getWeekDay(d2) * 24) /
                    self._domainType.hour.row(d2),
                );
              }
              return (
                Math.floor(d2.getHours() / self._domainType.hour.row(d2)) +
                self.getWeekDay(d2) * 4
              );
            }
          ...
          switch (self.options.domain) {
            ...
            case "month":
              return self.options.domainDynamicDimension &&
                ...
                ? self.getWeekNumber(
                    new Date(d2.getFullYear(), d2.getMonth() + 1, 0),
                  ) -
                    self.getWeekNumber(d2) +
                    1
                : 6;
            case "year":
              return self.options.domainDynamicDimension
                ? self.getWeekNumber(new Date(d2.getFullYear(), 11, 31)) -
                    self.getWeekNumber(new Date(d2.getFullYear(), 0)) +
                    1
                : 54;
          }
        },
        ...
        position: {
          x: function (d2) {
            switch (self.options.domain) {
              case "week":
                return Math.floor(
                  self.getWeekDay(d2) / self._domainType.day.row(d2),
                );
              case "month":
                if (self.options.colLimit > 0 || self.options.rowLimit > 0) {
                  return Math.floor(
                    (d2.getDate() - 1) / self._domainType.day.row(d2),
                  );
                }
                return (
                  self.getWeekNumber(d2) -
                  self.getWeekNumber(new Date(d2.getFullYear(), d2.getMonth()))
                );
              case "year":
                if (self.options.colLimit > 0 || self.options.rowLimit > 0) {
                  return Math.floor(
                    (self.getDayOfYear(d2) - 1) / self._domainType.day.row(d2),
                  );
                }
        ...
        position: {
          x: function (d2) {
            switch (self.options.domain) {
              case "year":
                return Math.floor(
                  self.getWeekNumber(d2) / self._domainType.week.row(d2),
                );
              case "month":
                return Math.floor(
                  self.getMonthWeekNumber(d2) / self._domainType.week.row(d2),
                );
            }
          },
          y: function (d2) {
    ...
    this._init = function () {
      self
        ...
        .map(function (d2) {
          self._domains.set(
            d2,
            self.getSubDomain(d2).map(function (d4) {
              return {
                t: self._domainType[self.options.subDomain].extractUnit(d4),
                v: null,
              };
            }),
          );
    ...
    this.paint = function (navigationDir) {
      ...
      rect
        ...
        .attr("class", function (d2) {
          return (
            "graph-rect" +
            self.getHighlightClassName(d2.t) +
            ...
            selection.attr("transform", function (d2) {
              ...
              switch (options.label.position) {
                case "right":
                  s +=
                    "translate(-" +
                    (w(d2) + self.domainHorizontalLabelWidth) +
                    " , " +
                    w(d2) +
                    ")";
                  break;
        ...
        domainSvg
          ...
          .attr("x", function (d2) {
            return options.verticalOrientation
              ? 0
              : self.domainPosition.getPosition(d2);
          })
          .attr("y", function (d2) {
            return options.verticalOrientation
              ? self.domainPosition.getPosition(d2)
  ...
  CalHeatMap.prototype = {
      ...
      if (d3.select(options.itemSelector)[0][0] === null) {
        throw new Error(
          "The node '" +
            ...
            "' specified in itemSelector does not exist",
        );
      }
      try {
    ...
    triggerEvent: function (eventName, successArgs, skip) {
      ...
      if (typeof this.options[eventName] === "function") {
        ...
        console.log(
          "Provided callback for " + eventName + " is not a function.",
        );
        return false;
      }
    },
      ...
      switch (this.options.domain) {
        case "hour":
          ...
          if (typeof range === "number" && domains.length < range) {
            if (range > 0) {
              domains.push(
                this.getHourDomain(domains[domains.length - 1], 2)[1],
              );
            } else {
      ...
      var _callback = function (error, data) {
        if (afterLoad !== false) {
          if (typeof afterLoad === "function") {
            ...
            console.log(
              "Provided callback for afterLoadData is not a function.",
            );
          }
        } else if (
    ...
    setLegend: function () {
      ...
      if (
        (arguments.length > 0 &&
          !arrayEquals(oldLegend, this.options.legend)) ||
        arguments.length >= 2
      ) {
