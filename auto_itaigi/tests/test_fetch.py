...
def test_fetch_moedict_entry_success():
    body = '{"h": [{"T": "han-tsî/han-tsû", "_": "8782", "d": [{"f": "`地~`瓜~"}]}]}'.encode("utf-8")
    mock_resp = _make_response(body)
    mock_urlopen = MagicMock()
    mock_urlopen.return_value.__enter__.return_value = mock_resp
...
def test_lookup_itaigi_tier1_fail_tier2_success():
    with (
        ...
        patch("auto_itaigi.utils.fetch_moedict_entry", return_value=("han-tsî", ["地瓜"], "https://r2-assets.moedict.tw/audio/t/8782.mp3")),
