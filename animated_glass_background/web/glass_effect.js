(function() {
  ...
  class GlassEffectBackground {
      constructor() {
          ...
          this.options = {
              threeD: {
                  ambientGlow: { innerColor: "rgba(118, 183, 229, 0.6)", innerOpacity: 0.8 },
                  electric: { enabled: true, arcCount: 4, arcThickness: 1.5, streakSpeedMultiplier: 1, width: 0.15, colors: { primary: "rgba(118, 183, 229, 0.8)", secondary: "rgba(180, 220, 255, 0.5)", tertiary: "rgba(255, 255, 255, 0.3)" } },
                  reflection: { speed: 0.03, intensity: 0.3, width: 0.3, color: "rgba(255,255,255,0.8)" }
              }
          };
      ...
      update(time) {
          ...
          const damping = 0.1;
          this.state.pointerSmoothed.x += (this.state.pointer.x - this.state.pointerSmoothed.x) * damping;
          this.state.pointerSmoothed.y += (this.state.pointer.y - this.state.pointerSmoothed.y) * damping;
      ...
      drawElectricTrails(radius) {
          const electric = this.options.threeD.electric;
          const palette = [electric.colors.primary, electric.colors.secondary, electric.colors.tertiary].filter(Boolean);
          ...
          palette.forEach((color, i) => {
              const offset = i / palette.length + this.state.continuousPhase * (electric.streakSpeedMultiplier || 1);
  ...
  if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', () => { window.glassEffectInstance = new GlassEffectBackground(); });
  } else {
