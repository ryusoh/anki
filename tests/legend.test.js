...
function validateLegend(mockDOM, expected, chartName) {
    ...
    expected.labels.forEach(label => {
        assert.ok(html.includes(label), `Legend must contain "${label}" for ${chartName}`);
    });
    
    expected.classes.forEach(cls => {
        assert.ok(html.includes(cls), `Legend must have class "${cls}" for ${chartName}`);
    });
    
    // Check display is set
    assert.strictEqual(legend.style.display, "flex", `Legend display must be "flex" for ${chartName}`);
}
...
function runTests() {
    ...
    try {
        ...
        console.log("   ✓ Due chart legend shows 未習熟 / 習熟済み with dot symbols");
        passed++;
    } catch (e) {
    ...
    try {
        ...
        console.log("   ✓ Reviews chart legend shows Reviews / Retention with dot symbols");
        passed++;
    } catch (e) {
    ...
    try {
        ...
        // Show due chart
        renderDueChart(mockDOM);
        assert.ok(mockDOM.chartLegend.innerHTML.includes("未習熟"), "Should have due legend");
        
        // Clear
        clearChart(mockDOM);
        assert.strictEqual(mockDOM.chartLegend.innerHTML, "", "Legend should be cleared");
        assert.strictEqual(mockDOM.chartLegend.style.display, "none", "Legend should be hidden");
    ...
    try {
        ...
        // Switch to reviews
        renderReviewsChart(mockDOM);
        validateLegend(mockDOM, REVIEWS_LEGEND_EXPECTED, "reviews chart (after switch)");
    ...
    try {
        ...
        assert.ok(dueHtml.includes('class="legend-color'), "Due legend must use legend-color class");
        
        // Check reviews chart
        renderReviewsChart(mockDOM);
        const reviewsHtml = mockDOM.chartLegend.innerHTML;
        assert.ok(reviewsHtml.includes('class="legend-color'), "Reviews legend must use legend-color class");
    ...
    try {
        ...
        // Due chart
        renderDueChart(mockDOM);
        assert.strictEqual(mockDOM.chartLegend.style.display, "flex", "Due legend should be visible");
        
        // Reviews chart
        renderReviewsChart(mockDOM);
        assert.strictEqual(mockDOM.chartLegend.style.display, "flex", "Reviews legend should be visible");
        
        // Clear
        clearChart(mockDOM);
        assert.strictEqual(mockDOM.chartLegend.style.display, "none", "Cleared legend should be hidden");
    ...
    try {
        ...
        assert.notStrictEqual(mockDOM.chartLegend, mockDOM.runningAmountCanvas, 
    ...
    try {
        ...
        sequence.forEach((chart, i) => {
            if (chart === "due") {
                renderDueChart(mockDOM);
                validateLegend(mockDOM, DUE_LEGEND_EXPECTED, `due chart (switch ${i + 1})`);
            } else {
                renderReviewsChart(mockDOM);
                validateLegend(mockDOM, REVIEWS_LEGEND_EXPECTED, `reviews chart (switch ${i + 1})`);
            }
        });
    ...
    try {
        ...
        renderReviewsChart(mockDOM);
        const reviewsHtml = mockDOM.chartLegend.innerHTML;
        assert.ok(/Reviews/.test(reviewsHtml), "Reviews legend should have English text");
        assert.ok(/Retention/.test(reviewsHtml), "Reviews legend should have English text");
    ...
    try {
        ...
        renderDueChart(mockDOM);
        const dueHtml = mockDOM.chartLegend.innerHTML;
        assert.ok(dueHtml.includes("color-young"), "Due should have young color class (blue)");
        assert.ok(dueHtml.includes("color-mature"), "Due should have mature color class (green)");
        
        // Reviews chart colors
        renderReviewsChart(mockDOM);
        const reviewsHtml = mockDOM.chartLegend.innerHTML;
        assert.ok(reviewsHtml.includes("color-reviews"), "Reviews should have reviews color class (blue)");
        assert.ok(reviewsHtml.includes("color-retention"), "Reviews should have retention color class (yellow)");
    ...
    if (failed > 0) {
        ...
        console.log("\n⚠️  Requirements:");
        console.log("   • Due chart: 未習熟 / 習熟済み (Japanese, blue/green dots)");
        console.log("   • Reviews chart: Reviews / Retention (English, blue/yellow dots)");
        ...
        console.log("   • Reviews chart legend: Reviews / Retention with dot symbols");
