...
const { TrieNode, CommandTrie, createCommandTrie } = (() => {
    ...
    function createCommandTrie() {
        ...
        trie.insertAll([
            ...
            'help', 'h', '?', 'charts', 'list', 'clear', 'cls', 'c',
            'plot', 'p', 'plot due', 'pd', 'plot reviews', 'pr',
            'due', 'd', 'future', 'f', 'reviews', 'r',
            'show', 's', 'show due', 'sd', 'show reviews', 'sr',
            // Range shortcuts
            '1m', '2m', '3m', '6m', '1y', '2y', '3y', '5y', '10y', 'all'
        ]);
...
function runTests() {
    ...
    console.log("\n📋 Test 1: Valid commands are recognized");
    const validCommands = ['help', 'plot due', 'plot reviews 3m', 'due', 'reviews', '1m', 'all'];
    ...
    const invalidCommands = ['unknown', 'foo bar', 'plot foo', 'invalid123', 'xyz'];
    invalidCommands.forEach(cmd => {
        try {
            const result = trie.validate(cmd);
            assert.strictEqual(result.valid, false, `${cmd} should be invalid`);
            assert.strictEqual(result.isPartial, false, `${cmd} should not be partial match`);
            assert.strictEqual(result.suggestions.length, 0, `${cmd} should have no suggestions`);
    ...
    partialTests.forEach(({ prefix, expectedContains }) => {
        try {
            ...
            assert.strictEqual(result.valid, false, `${prefix} should not be exact match`);
            assert.strictEqual(result.isPartial, true, `${prefix} should be partial match`);
            assert.ok(result.suggestions.length > 0, `${prefix} should have suggestions`);
            assert.ok(result.suggestions.some(s => s.includes(expectedContains)), 
                `Suggestions should contain "${expectedContains}"`);
            console.log(`   ✓ "${prefix}" → ${result.suggestions.length} suggestions`);
            passed++;
        } catch (e) {
    ...
    autocompleteTests.forEach(({ prefix, minSuggestions }) => {
        try {
            ...
            assert.ok(suggestions.length >= minSuggestions, 
    ...
    const futureCommands = ['plot newchart', 'analyze', 'stats', 'export data', 'import'];
    futureCommands.forEach(cmd => {
        try {
            const result = trie.validate(cmd);
            assert.strictEqual(result.valid, false, `${cmd} should be invalid (not registered)`);
            assert.strictEqual(result.isPartial, false, `${cmd} should not be partial match`);
    ...
    caseTests.forEach(cmd => {
        try {
            ...
            assert.strictEqual(result.valid, expectedResult, `${cmd} should match case-insensitively`);
    ...
    try {
        ...
        assert.ok(emptyResult.valid || emptyResult.suggestions.length > 0, 'Empty should return all commands');
        assert.ok(spaceResult.valid || spaceResult.suggestions.length > 0, 'Whitespace should return all commands');
    ...
    try {
        ...
        assert.strictEqual(allCommands.length, trie.size(), 'getAllCommands should match size');
    ...
    try {
        ...
        assert.strictEqual(trie.search('test command'), true, 'Should find inserted command');
    ...
    try {
        ...
        assert.strictEqual(validCount, allCommands.length, 'All registered commands should be valid');
