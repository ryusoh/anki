...
function runTests() {
  ...
  REQUIRED_RANGES.forEach((range) => {
    try {
      assert.ok(
        TIME_RANGES[range] !== undefined,
        `Range ${range} must be defined`,
      );
      console.log(
        `   ✓ ${range}: ${TIME_RANGES[range] === null ? "all" : TIME_RANGES[range] + " days"}`,
      );
      passed++;
    } catch (e) {
  ...
  aliases.forEach((alias) => {
    REQUIRED_RANGES.slice(0, 3).forEach((range) => {
      // Test first 3 ranges for each alias
      try {
        const result = parseCommand(`${alias} ${range}`);
        assert.strictEqual(
          result.command,
          "due",
          "Should parse as due command",
        );
        assert.strictEqual(
          result.range,
          range,
  ...
  try {
    ...
    console.log(
      `   ✓ TIME_RANGES has all ${REQUIRED_RANGES.length} required keys`,
    );
    passed++;
  } catch (e) {
  ...
  invalidRanges.forEach((range) => {
    try {
      ...
      assert.strictEqual(
        result.range,
        range,
        "Should capture the invalid range",
      );
  ...
  Object.entries(TIME_RANGES)
    ...
    .forEach(([range, days]) => {
      try {
        ...
        assert.ok(
          filtered.every((d) => "day" in d && "mature" in d && "young" in d),
  ...
  console.log(
    `   ✓ All ${chartCommands.length * REQUIRED_RANGES.length} command/range combinations supported`,
  );
  ...
  if (failed > 0) {
    console.log(
      "❌ TESTS FAILED - Some time range filters are not working correctly",
    );
