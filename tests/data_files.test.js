...
// Files that MUST be in project root for web access
...
function runTests() {
    ...
    // Test 1: Web-accessible files exist in root
    console.log("\n📋 Test 1: Web-accessible files in project root");
    WEB_ACCESSIBLE_FILES.forEach(file => {
        const filePath = path.join(PROJECT_ROOT, file);
        try {
            assert.ok(fs.existsSync(filePath), `File not found: ${file}`);
    ...
    // Test 2: Web files are NOT in data/anki/ (prevent duplication)
    console.log("\n📋 Test 2: Web files not duplicated in data/anki/");
    WEB_ACCESSIBLE_FILES.forEach(file => {
        const dataPath = path.join(DATA_ROOT, file);
        try {
            assert.ok(!fs.existsSync(dataPath), `Should not exist in data/anki/: ${file}`);
            console.log(`   ✓ ${file} not in data/anki/`);
            passed++;
        } catch (e) {
            console.log(`   ✗ ${file}: ${e.message}`);
            failed++;
    ...
    REQUIRED_DIRS.forEach(dir => {
        ...
        try {
            assert.ok(fs.existsSync(dirPath), `Directory not found: data/anki/${dir}`);
    ...
    const jsonValidations = [
        {
            ...
            validate: (data) => {
                ...
                assert.ok(data.futureDue[0].mature !== undefined, 'Missing mature field');
        ...
        },
        {
            ...
            validate: (data) => {
                ...
                assert.ok(data.reviews[0].retention !== undefined, 'Missing retention field');
            }
        }
    ];
    ...
    try {
        ...
        assert.ok(validFiles.length >= 12, 'Should have at least 12 months of data');
        ...
        console.log(`   ✓ ${validFiles.length} monthly files (${oldest} to ${newest})`);
        passed++;
    } catch (e) {
    ...
    console.log("   ✓ Web files must be in project root");
    ...
    if (failed > 0) {
        ...
        console.log("\n⚠️  All web-accessible data files must be in project root:");
        WEB_ACCESSIBLE_FILES.forEach(f => console.log(`   - ${f}`));
        console.log("\n⚠️  Internal data files must be in data/anki/:");
        DATA_DIR_FILES.forEach(f => console.log(`   - ${f}`));
        console.log();
        process.exit(1);
    } else {
        console.log("✅ ALL TESTS PASSED - Data file structure correct");
        console.log("\n📝 Conventions enforced:");
        console.log("   • Web files (*.json for HTTP access) → project root");
