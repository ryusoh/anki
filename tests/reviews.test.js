...
function groupAndSortDecks(byDeckData, showTime) {
    ...
    for (const [deckName, entries] of Object.entries(byDeckData)) {
        ...
        const total = entries.reduce((sum, e) => sum + (showTime ? (e.time || 0) : (e.count || 0)), 0);
        if (total === 0) continue;
    ...
    const sortedGroups = Object.keys(groups).sort((a, b) => groups[b].total - groups[a].total);
...
function getGroupedDeckColor(groupIndex, subIndex, totalInGroup) {
    ...
    const match = baseColor.match(/hsla\((\d+),\s*(\d+)%,\s*([\d.]+)%,\s*([\d.]+)\)/);
    ...
    l = Math.max(25, Math.min(85, l + lightOffsets[subIndex % lightOffsets.length]));

    const newS = Math.max(40, Math.min(100, s + satOffsets[subIndex % satOffsets.length]));
