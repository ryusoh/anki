...
def overview_table(self):
  ...
  if(daysUntilDone == 1):
    cards['daysLeft'] = '{} 日'.format(daysUntilDone)
  else:
    cards['daysLeft'] = '{} 日'.format(daysUntilDone)
  ...
  output_table += u'''
    ...
    <tr>
      ...
      <td class="row4 percent"><span style="font-weight:bold">{percent[total]:.0%}</span></td>
    </tr>
      <td colspan="3"><hr /></td>
    <tr>
  ...
  if deck_is_finished:
    if (config == None or not 'options' in config) or (config['options'].get(
